...
def menu_def():
    ...
    settings = sc.blit(btn_stg.image, btn_stg.rect)
...
btn_stg = Game_sprite(200, "settings.png", settings_group)
...
while game:
    ...
    score=pygame.font.Font(None,89)
        ...
        if i.type==pygame.MOUSEBUTTONDOWN:
            if i.button==1:
                ...
                if play.collidepoint(pos):
                    ...
                    var1 = IntVar()
                    rb = Radiobutton(root, text="Бессмертие", variable=var1, value=1)
                    rb.pack()
                    btn_st = Button(root, text = "Принять",command=btnclose)
                    btn_st.pack()
                    root.mainloop()
    ...
    if game_over==1:
        ...
        if car2.rect.y < H:
            car2.rect.y = car2.rect.y + randint(1,4)
        else:
            car2 = Game_sprite(randint(100,300), 'Car2.png', cars)
            car2.rect.y = 0
            s=s+1
            text_score=score.render(str(s),100,(255,255,255))
